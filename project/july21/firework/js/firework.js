	...
	state: {
		...
		config: {
			...
			size: IS_DESKTOP
				? '4' // Desktop default
				: IS_HEADER 
					? '1' // Profile header default (doesn't need to be an int)
					: '3', // Mobile default
			autoLaunch: true,
			finale: false,
...
const helpContent = {
	shellType: {
		header: 'Тип фейерверка',
		body: 'Тип фейерверка, который будет запущен. Выберите "случайный" для приятного ассортимента!'
	},
	shellSize: {
		header: 'Размер фейерверка',
		body: 'Размер фейерверка. Смоделированные по образцу реальных размеров снарядов фейерверка, более крупные снаряды имеют более крупные всплески с большим количеством звезд, а иногда и более сложные эффекты.'
	},
	quality: {
		header: 'Качество',
		body: 'Общее качество графики. Если анимация не работает гладко, попробуйте снизить качество. Высокое качество значительно увеличивает количество выдаваемых искр и может привести к запаздыванию.'
	},
	skyLighting: {
		header: 'Освещение неба',
		body: 'Освещает фон, как взрываются фейерверки. Если фон на экране выглядит слишком ярким, попробуйте установить его в положение "тусклый" или "нет".'
	},
	scaleFactor: {
		header: 'Масштаб',
		body: 'Позволяет масштабировать размер всех фейерверков, по существу перемещая вас ближе или дальше. Для больших размеров корпуса может быть удобно немного уменьшить масштаб, особенно на телефонах или планшетах.'
	},
	autoLaunch: {
		header: 'Автоматический огонь',
		body: 'Запускает последовательности фейерверков автоматически. Сядьте поудобнее и наслаждайтесь шоу, или отключите его, чтобы иметь полный контроль.'
	},
	finaleMode: {
		header: 'Финальный режим',
		body: 'Запускает интенсивные взрывы фейерверков. Может вызвать задержку. Требуется включить функцию "автоматический огонь".'
	},
	hideControls: {
		header: 'Скрыть элементы управления',
		body: 'Скрывает полупрозрачные элементы управления в верхней части экрана. Полезно для скриншотов или просто для более плавного опыта. Находясь в скрытом состоянии, вы все равно можете нажать на правый верхний угол, чтобы снова открыть это меню.'
	},
	fullscreen: {
		header: 'Полноэкранный режим',
		body: 'Переключает полноэкранный режим.'
	},
	longExposure: {
		header: 'Открытый затвор',
		body: 'Экспериментальный эффект, который сохраняет длинные полосы света, похож на оставление затвора камеры открытым.'
	}
};
...
function init() {
	...
	setOptionsForSelect(appNodes.quality, [
		{ label: 'Низкое', value: QUALITY_LOW },
		{ label: 'Нормальное', value: QUALITY_NORMAL },
		{ label: 'Высокое', value: QUALITY_HIGH }
	]);
	
	setOptionsForSelect(appNodes.skyLighting, [
		{ label: 'Нет', value: SKY_LIGHT_NONE },
		{ label: 'Тусклое', value: SKY_LIGHT_DIM },
		{ label: 'Нормальное', value: SKY_LIGHT_NORMAL }
	]);
